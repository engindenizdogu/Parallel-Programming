...
{
    // Ok, this is cheating.
    return random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() + random_float() - 6.f;
}
...
void node_update_low_res_single(Node *node)
{
    if (node->is_leaf)
    {
        ...
        // need the high-resolution data at all. In that case the low-resulution data is transmitted
        // via MPI.
        if (node->timestamp_low_res > node->leaf.timestamp_high_res)
            return;
    ...
    {
        if (node->tree.lower_left->timestamp_low_res != node->tree.lower_right->timestamp_low_res || node->tree.lower_right->timestamp_low_res != node->tree.upper_left->timestamp_low_res || node->tree.upper_left->timestamp_low_res != node->tree.upper_right->timestamp_low_res)
        {
        ...
        node->low_res_x = node->tree.lower_left->low_res_x * node->tree.lower_left->low_res_mass + node->tree.lower_right->low_res_x * node->tree.lower_right->low_res_mass + node->tree.upper_left->low_res_x * node->tree.upper_left->low_res_mass + node->tree.upper_right->low_res_x * node->tree.upper_right->low_res_mass;
        node->low_res_y = node->tree.lower_left->low_res_y * node->tree.lower_left->low_res_mass + node->tree.lower_right->low_res_y * node->tree.lower_right->low_res_mass + node->tree.upper_left->low_res_y * node->tree.upper_left->low_res_mass + node->tree.upper_right->low_res_y * node->tree.upper_right->low_res_mass;
        node->low_res_mass = node->tree.lower_left->low_res_mass + node->tree.lower_right->low_res_mass + node->tree.upper_left->low_res_mass + node->tree.upper_right->low_res_mass;
        if (node->low_res_mass > 0.001f)
        {
            node->low_res_x /= node->low_res_mass;
            node->low_res_y /= node->low_res_mass;
...
Node *node_create_helper(int num_leaves, int points, Node *parent, float x0, float y0, float x1, float y1, int *id_leaf, int *id_tree)
{
    ...
    else if (num_leaves == 1)
    {
        ...
        if (actual_points < points / 4)
            actual_points = points / 4;
...
Node *node_find(Node *root, int id)
{
    ...
    {
        Node *node;
        if ((node = node_find(root->tree.lower_left, id)))
            return node;
        if ((node = node_find(root->tree.lower_right, id)))
            return node;
        if ((node = node_find(root->tree.upper_left, id)))
            return node;
        if ((node = node_find(root->tree.upper_right, id)))
            return node;
        return NULL;
    }
}
